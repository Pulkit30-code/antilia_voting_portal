...
export default async function SystemLoginPage() {
  ...
  if (session?.role === "SYSTEM") redirect("/system");

  return (
    <main className="relative flex min-h-screen items-center justify-center overflow-hidden bg-[#0e0e0d] px-5 py-10 sm:px-8">
      <div aria-hidden="true" className="absolute inset-0 bg-[radial-gradient(circle_at_18%_18%,rgba(185,154,95,0.13),transparent_30%),radial-gradient(circle_at_84%_82%,rgba(185,154,95,0.06),transparent_34%)]" />
      ...
      <section className="relative z-10 w-full max-w-[460px] animate-[fadeIn_.45s_ease-out]">
        ...
        <div className="rounded-2xl border border-white/[0.09] bg-[#181816]/95 p-6 shadow-[0_30px_100px_rgba(0,0,0,.5)] backdrop-blur-xl sm:p-9">
          <div className="mb-8">
            ...
            <p className="text-[10px] font-bold uppercase tracking-[0.2em] text-[#b99a5f]">System Access</p>
            <h1 className="mt-2 font-serif text-3xl leading-tight text-[#f7f2e9] sm:text-[2.15rem]">System Administration</h1>
            <p className="mt-3 text-sm leading-6 text-[#9e968a]">Authenticate to manage the current election state and protected system controls.</p>
          </div>

          <AdminLoginForm role="SYSTEM" redirectTo="/system" theme="dark" />
          ...
          <div className="mt-7 flex items-center justify-center gap-2 border-t border-white/[0.07] pt-6 text-xs text-[#777064]">
            ...
            <span>Restricted System Control session</span>
          </div>
        </div>
